...
read.Data <- function(illum_data, od_data, design_matrix){
  illumData <- read.csv(paste0(projectFolder,illum_data), header=F)
  odData <- read.csv(paste0(projectFolder,od_data), header=F)
  designMatrix <- read.csv(paste0(projectFolder,design_matrix), header=F)
  designMatrix <- as.matrix(designMatrix, header=F,
                             check.names=F, col.names=unique(colnames(design_matrix)), 
                             row.names=unique(rownames(design_matrix)))
  return(designMatrix)
}
